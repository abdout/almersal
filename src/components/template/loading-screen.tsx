...
export function LoadingScreen({ isContentLoaded, onComplete }: LoadingScreenProps) {
  ...
  useEffect(() => {
    ...
    // If content is loaded and we're past 90%, accelerate to 100
    if (isContentLoaded && count >= 90 && count < 100) {
    ...
    // If content is loaded but we're below 90%, speed up moderately
    if (isContentLoaded && count < 90) {
      const timeout = setTimeout(() => {
    ...
    // Pause at 90% if content not loaded
    if (count >= 90 && !isContentLoaded) {
      return;
    }
    ...
    return () => clearTimeout(timeout);
  }, [count, isContentLoaded, isExiting, startExitAnimation]);
